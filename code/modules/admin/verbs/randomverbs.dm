...
/client/proc/admin_headset_message(mob/M in GLOB.mob_list, sender = null)
	...
	to_chat(H, "You hear something crackle in your ears for a moment before a voice speaks.  \"Please stand by for a message from [sender == "Syndicate" ? "your benefactor" : "Central Command"].  Message as follows[sender == "Syndicate" ? ", agent." : ":"] <span class='bold'>[input].</span> Message ends.\"", confidential = TRUE)
...
/client/proc/cmd_admin_toggle_fov()
	...
	if(busy_toggling_fov)
		to_chat(usr, "<span class='warning'>A previous call of this function is still busy toggling FoV [on_off ? "on" : "off"]. Have some patiece</span>.", confidential = TRUE)
		return
...
/obj/effect/temp_visual/target/proc/fall(list/flame_hit)
	...
	for(var/mob/living/L in T.contents)
		...
		if(islist(flame_hit) && !flame_hit[L])
			L.adjustFireLoss(40)
			to_chat(L, "<span class='userdanger'>You're hit by the drake's fire breath!</span>", confidential = TRUE)
			flame_hit[L] = TRUE
		else
